...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_SRAsearch_fasterq_dump.txt", 'r+')
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_SRAsearch_fasterq_dump"+str(count)+".txt", 'w+')
    e.write(lines)
...
commands = ["./fasterq_dump.exe -s 30" for i in range(num_run)]
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_SRAsearch_fasterq_dump*"):
    os.remove(filename) 
...
with open('fasterq_dump_output.txt', 'w') as filehandle:
