...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_SRAsearch_merge2.txt", 'r+')
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_SRAsearch_merge2"+str(count)+".txt", 'w+')
    e.write(lines)
...
commands = ["./merge2.exe --niter 200" for i in range(num_run)]
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_SRAsearch_merge2*"):
    os.remove(filename) 
...
with open('merge2_output.txt', 'w') as filehandle:
