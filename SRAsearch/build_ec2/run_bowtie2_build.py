...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_SRAsearch_bowtie2_build.txt", 'r+')
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_SRAsearch_bowtie2_build"+str(count)+".txt", 'w+')
    e.write(lines)
...
commands = ["./bowtie2_build.exe --niter 330" for i in range(num_run)]
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_SRAsearch_bowtie2_build*"):
    os.remove(filename) 
...
with open('bowtie2_build_output.txt', 'w') as filehandle:
