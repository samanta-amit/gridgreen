...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_SRAsearch_bowtie2.txt", 'r+')
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_SRAsearch_bowtie2"+str(count)+".txt", 'w+')
    e.write(lines)
...
commands = ["./bowtie2.exe -s 30" for i in range(num_run)]
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_SRAsearch_bowtie2*"):
    os.remove(filename) 
...
with open('bowtie2_output.txt', 'w') as filehandle:
