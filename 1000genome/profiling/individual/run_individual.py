...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_1000genome_individual.txt", 'r+')
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_1000genome_individual"+str(count)+".txt", 'w+')
    e.write(lines)
...
commands = ["./individual.exe -s 13" for i in range(num_run)]
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_1000genome_individual*"):
    os.remove(filename) 
...
with open('individual_output.txt', 'w') as filehandle:
