...
lam_red_list = ['sifting', 'individual_merge']
lam_red_exe = [0.9600190901756287, 0.7688855576515198]
lam_red_read = [0.09549355506896973, 0.09685873985290527]
lam_red_write = [0.04701089859008789, 0.031760454177856445]

# Carbon footprint constants
CARBON_INTENSITY_US_WEST_1 = 200
...
def speculative_warmup(phase_idx):
    ...
    for app_name in next_apps:
        if app_name in lam_red_list:
            threading.Thread(target=run_lam, args=(app_name, 0, 'us-west-1', True, False)).start()

def run_lam(name, num, region='us-west-1', warmup=False, is_optimization=False):
    ...
    if cache_key in lam_cache and not warmup and not is_optimization:
        ...
        if num > 0:
            for i in range(1, num + 1):
                command = f"aws lambda invoke --function-name {name} --region {region} --payload '{{\"key1\":\"{i}\"}}' {name}_response{i}.txt"
...
def ssh_connect_with_retry_with_run(ssh, ip_address, retries, name, num):
    if retries > 3:
        return False
    privkey = paramiko.RSAKey.from_private_key_file('/users/amit049/NewKeyPair.pem')
    try:
        ssh.connect(hostname=ip_address, timeout=7200, username='ec2-user', pkey=privkey)
    except Exception as e:
        time.sleep(5)
...
def run_ec2(name, num, is_optimization=False):
    ...
    if cache_key in ec2_cache and not is_optimization:
        ...
        carbon = energy_kwh * CARBON_INTENSITY_US_WEST_1
    ...
    try:
        ...
        for j, (app_name, num_ec2, num_lam) in enumerate(zip(apps, num_ec2_list, num_lam_list)):
            if num_ec2 > 0:
                ...
                threads.append(t1)
            if num_lam > 0:
                region = lam_regions.get(app_name, 'us-west-1')
...
def optimize_phase(args):
    ...
    for j, (app_name, num_tasks) in enumerate(zip(apps, total_tasks)):
        ...
        ec2_total_time = ec2_time + ec2_queue_time
        ec2_cost = ec2_time * cost_of_ec2
        ec2_score = ec2_total_time + ec2_carbon

        lam_time, lam_carbon, _ = run_lam(app_name, 1, 'us-west-1', False, is_optimization=True)
        ...
        if ec2_score < lam_score * threshold and ec2_cost * num_tasks <= remaining_cost and num_nodes > sum(ec2_counts):
            ...
            lam_regions[app_name] = 'us-west-1'
    ...
    for j, platform, score, unit_time, unit_carbon, unit_cost in sorted(decisions, key=lambda x: x[2]):
        ...
        if platform == 'ec2' and num_nodes > sum(ec2_counts) and unit_cost * num_tasks <= remaining_cost:
            ...
            lam_regions[app_name] = lam_regions.get(app_name, 'us-west-1')
    ...
    for num_nodes in num_nodes_options:
        ...
        for phase_idx in range(phase):
            ...
            for j, (app_name, num_tasks) in enumerate(zip(apps, total_tasks)):
                ...
                if ec2_total_time < lam_time and ec2_cost * num_tasks <= remaining_cost and num_nodes > sum(ec2_counts):
                    decisions.append((j, 'ec2', ec2_total_time, ec2_time, ec2_carbon, ec2_cost))
                else:
...
def run_mashup_service_based():
    ...
    for phase_idx in range(phase):
        ...
        for app_name, num_tasks in zip(apps, total_tasks):
            ...
            ec2_service_time = time.time() - start_time
            ssh.close()

            _, _, lam_service_time = run_lam(app_name, 1, 'us-west-1', False, True)

            max_ec2_tasks = min(num_tasks, num_nodes - sum(ec2_counts)) if num_nodes > sum(ec2_counts) else 0
            if ec2_service_time < lam_service_time and max_ec2_tasks > 0:
...
def reset_globals():
    ...
    global ec2_total_time, lam_total_time, ec2_service_time, lam_service_time, ec2_app_times, ec2_app_carbon, lam_app_times, lam_app_carbon, component_carbon_log
...
if __name__ == "__main__":
    app_name = "1000genome"
    phase = 3
    app = [['individual'], ['individual_merge', 'sifting'], ['mutation_overlap', 'frequency']]
    num_of_app = [[1252], [1, 1], [626, 626]]
    num_in_ec2 = [[0], [1, 1], [0, 0]]

    instance_ID = 'i-061936bb55db4f191'
    ec2_name = "m5a.8xlarge"
    cost_of_lambda = 0.00005
    s3_cost = 8.75190259e-9
    ec2_cost_list = [0.086, 0.172, 0.344, 0.688, 1.376, 2.064, 2.752, 4.128]
    ec2_name_list = ['m5a.large', 'm5a.xlarge', 'm5a.2xlarge', 'm5a.4xlarge', 'm5a.8xlarge', 'm5a.12xlarge', 'm5a.16xlarge', 'm5a.24xlarge']
    cost_of_ec2 = ec2_cost_list[ec2_name_list.index(ec2_name)] / 3600
    num_in_lam = [[num_of_app[i][j] - num_in_ec2[i][j] for j in range(len(num_of_app[i]))] for i in range(len(num_of_app))]
    ...
    for phase_idx, apps in enumerate(app):
        for app_name in apps:
            num_tasks = num_of_app[phase_idx][apps.index(app_name)]
            lambda_carbon_sim[app_name] = simulate_lambda_carbon(app_name, min(num_tasks, 10))
