...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_epigenomics_sol2sanger.txt", 'r+')   ##
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_epigenomics_sol2sanger"+str(count)+".txt", 'w+') ##
    e.write(lines)
...
commands = ["./sol2sanger.exe -s 23" for i in range(num_run)]   ##
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_epigenomics_sol2sanger*"): ##
    os.remove(filename) 
...
with open('sol2sanger_output.txt', 'w') as filehandle:  ##
