...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_epigenomics_chr21.txt", 'r+')   ##
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_epigenomics_chr21"+str(count)+".txt", 'w+') ##
    e.write(lines)
...
commands = ["./chr21.exe --niter 270" for i in range(num_run)]   ##
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_epigenomics_chr21*"): ##
    os.remove(filename) 
...
with open('chr21_output.txt', 'w') as filehandle:  ##
