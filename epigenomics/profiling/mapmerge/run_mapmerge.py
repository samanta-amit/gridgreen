...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_epigenomics_mapmerge.txt", 'r+')   ##
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_epigenomics_mapmerge"+str(count)+".txt", 'w+') ##
    e.write(lines)
...
commands = ["./mapmerge.exe -s 45" for i in range(num_run)]   ##
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_epigenomics_mapmerge*"): ##
    os.remove(filename) 
...
with open('mapmerge_output.txt', 'w') as filehandle:  ##
