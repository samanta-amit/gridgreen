...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_epigenomics_pileup.txt", 'r+')   ##
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_epigenomics_pileup"+str(count)+".txt", 'w+') ##
    e.write(lines)
...
commands = ["./pileup.exe -s 46" for i in range(num_run)]   ##
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_epigenomics_pileup*"): ##
    os.remove(filename) 
...
with open('pileup_output.txt', 'w') as filehandle:  ##
