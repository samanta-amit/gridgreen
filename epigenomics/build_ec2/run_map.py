...
def run(command,count):
    ...
    start_time=time.time()
    f= open("/tmp/read_file_epigenomics_map.txt", 'r+')   ##
    lines=f.read()
    read_list[count] = time.time() - start_time
   
    start_time=time.time()
    e=open("/tmp/write_file_epigenomics_map"+str(count)+".txt", 'w+') ##
    e.write(lines)
...
commands = ["./map.exe -s 21" for i in range(num_run)]   ##
threads=[]
count=0
...
for filename in glob.glob("/tmp/write_file_epigenomics_map*"): ##
    os.remove(filename) 
...
with open('map_output.txt', 'w') as filehandle:  ##
